...
for _ in range(T):
    ...
    # 접근 방식
    # 1. 정렬하기 : 서류 등수를 기준으로 오름차순 정렬한다.정렬하고 나면 면접 점수만 비교하면 됨
    # 2. 비교하기 : 반복문 돌면서 면접 등수가 기준보다 높으면 채용 가능하므로 카운트 해줌(면접 등수가 더 낮다? 서류 등수도 더 낮으니까 채용 안됨)
    # 3. 기준 바꿔주기 : 카운트 했으면 기준을 카운트 한 사람의 면접 등수로 바꿔줌.
    # 근데 왜안되지
    
    N = int(input())
    lanks = [list(map(int, input().split())) for _ in range(N)]
    lanks.sort(key=lambda x:x[0])
    highst_lank = 100000
    result = 0
    for lank in lanks:
        if lank[1] < highst_lank:
            highst_lank = lank[1]
            result += 1
    print(result)
